...
COS_distance=150;//Set spawn distance
_aerielActivation=true;// Set if flying units can activate civilian Zones

//blackListTowns = [];

blackListTowns = ["La Passagne","Saint Jean","Arudy","Dourdan","Houdan","Sainte Marie","Le Port","Military Base","La Riviere"];// Remove towns from COS

whiteListMkrs= ["COS_1", "COS_2", "COS_3", "COS_4", "COS_5", "COS_6", "COS_7", "COS_8", "COS_9", "COS_10", "COS_11", "COS_12", "COS_13", "COS_14", "COS_15", "COS_16"];// Add Custom Markers for COS to populate
...
COSmotPool =["I_MBT_03_cannon_F","I_G_Offroad_01_armed_F","I_G_Offroad_01_armed_F","I_APC_tracked_03_cannon_F","I_MRAP_03_hmg_F"];

COSmaxGrps=4;//Set Maximum group limit for COS at any one time (If limit is hit then civilians will be placed into a single group for each town)
...
cosMkrArray=[];
server setvariable ["cosGrpCount",4];//Set global group count
_rad=175;// Radius increase increment for finding minimum spawn points
