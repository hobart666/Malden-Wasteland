...
_respawnTimer =   PARAM(2, 60*60);  // Default respawn timer when vehicle is broken (0 = respawn immediatly (not recommended))
_desertedTimer =  PARAM(3, 60*60);  // Default respawn timer when vehicle is deserted but not broken
_proxyTimer =     PARAM(4, 60*60);  // Respawn timer when the last vehicle owner is within the defined proximity distance
_proxyDistance =  PARAM(5, 5000);   // Distance from the vehicle within which the last owner triggers the proximity timer
_minDistance =    PARAM(6, 10000);     // Minimum displacement distance from original location before the respawn timer is auto-activated (0 = only respawn when broken or looted)
