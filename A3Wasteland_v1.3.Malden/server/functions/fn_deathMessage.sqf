...
else
{
	...
	private _message = switch (_cause) do
	{
		...
		case "suicide": { format ["%1 commited suicide", _victim] }; // respawned from pause menu, crashed vehicle, last resort, player setDamage 1

		case "drown": { format ["%1 succumbed to the elements", _victim] }; // ran out of oxygen underwater, or sank while bleeding
		...
		default
		{
			switch (true) do
			{
				case (_aiKiller):           { format ["%1 got p00ned", _victim] }; // vehicle destroyed by AI
				case (_killer != ""): 
				{
